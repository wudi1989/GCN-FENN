    ...
    for learning_rate in [0.0001,0.0005,0.00001,0.00005]:
        for dim in [16,8,32]:
            for _ in range(10):
                ...
                model = nn.Sequential(MyGCN(31, dim), MLP(2*dim, 32, 16, 8, 2))
                ...
                for epoch in range(args.epochs):
                    ...
                    features = torch.FloatTensor(matrix_array[19])

                    adj = torch.zeros([23, 18], dtype=torch.long)

                    df = pd.read_excel("液化气——地区间的贸易流向/天然气：2019年液化天然气贸易流向.xlsx")
                    labels = list(df.columns.values)
                    ...
                    df = pd.read_excel("液化气——地区间的贸易流向/天然气：2020年液化天然气贸易流向.xlsx")
                    labels = list(df.columns.values)
                    ...
                    if tp!=0 and fn!=0:
                        ...
                        if tp+fn>414*0.82:
                            ...
                            if 2*(tp/(tp+261-fn))*(tp/153)/(tp/153+tp/(tp+261-fn))>mx[0][3]:
                                mx[0][1]=tp/(tp+261-fn)
                                mx[0][2]=tp/153
                                mx[0][3]=2*(tp/(tp+261-fn))*(tp/153)/(tp/153+tp/(tp+261-fn))
                                final_predict = output[:, 1].detach().numpy()
                                final_label = label.detach().numpy()
                ...
                print(f"time_step:2020,{mx[0][0]},{mx[0][1]},{mx[0][2]},{mx[0][3]},{mx[0][4]},{mx[0][5]}")
